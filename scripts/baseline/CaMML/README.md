...
- `experiment-config/run/run_LLM_CaMML.sh`: Merged batch launcher for the CaMML experiments.
...
1. Install the required packages by `sh install.sh` (Python dependencies only; reads `requirements.txt` in the same directory)
2. Initialize external repositories and local folders by `sh setup.sh`
3. Use the command `sh /Users/xiaoyuhe/Causal-LLM/experiment-config/run/run_LLM_CaMML.sh` to execute the merged CaMML batch launcher.
