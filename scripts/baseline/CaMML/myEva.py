...
class MetricsDAG(object):
    ...
    def _count_accuracy(B_est, B_true, decimal_num=4):
        ...
        mt = {'extra': len(extra_lower), 'missing': len(missing_lower), 'reverse': len(reverse), 'fdr': fdr, 'tpr': tpr, 'fpr': fpr, 'shd': shd, 'nnz': pred_size, 'precision': precision, 'recall': recall, 'F1': F1, 'gscore': gscore}
        for i in mt:
    ...
    def _cal_gscore(W_p, W_true):
        ...
        # true_positives
        num_tp = np.sum((W_p + W_true) == 2)
        # False Positives + Reversed Edges
        num_fn_r = np.sum((W_p - W_true) == 1)
