...
from helpers.tf_compat import tf
...
def main():
    ...
    output_dir = os.environ.get(
        "KCRL_OUTPUT_DIR",
        'output/{}'.format(datetime.now(timezone('Asia/Hong_Kong')).strftime('%Y-%m-%d_%H-%M-%S-%f')[:-3]),
    )
    ...
    # Keep the original defaults available, but let external runners pass a
    # processed dataset directory through environment variables.
    config.read_data = str(os.environ.get('KCRL_READ_DATA', '1')).lower() not in {
        '0',
        'false',
        'no',
    }
    config.data_path = os.environ.get('KCRL_DATA_PATH', config.data_path)
    config.score_type = os.environ.get('KCRL_SCORE_TYPE', config.score_type)
    config.reg_type = os.environ.get('KCRL_REG_TYPE', config.reg_type)
    config.lambda_flag_default = str(
        os.environ.get('KCRL_LAMBDA_FLAG_DEFAULT', '1')
    ).lower() not in {
        '0',
        'false',
        'no',
    }
    config.nb_epoch = int(os.environ.get('KCRL_NB_EPOCH', config.nb_epoch))
    config.input_dimension = int(
        os.environ.get('KCRL_INPUT_DIMENSION', config.input_dimension)
    )
    config.lambda_iter_num = int(
        os.environ.get('KCRL_LAMBDA_ITER_NUM', config.lambda_iter_num)
    )

    x_file_path = '{}/X.npy'.format(config.data_path)
    bundle_file_path = '{}/data.npy'.format(config.data_path)
    file_path = x_file_path if os.path.exists(x_file_path) else bundle_file_path
    if os.path.exists(file_path):
        observed = np.load(file_path, allow_pickle=file_path.endswith('data.npy'))
        if file_path.endswith('data.npy'):
            observed = observed.item().get('x')
        if observed is not None and observed.ndim == 2:
            config.data_size = observed.shape[0]
            config.max_length = observed.shape[1]
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        if last_acc_est is None or last_acc_est2 is None:
            final_graph = graphss[-1]
            final_graph_pruned = final_graph
            if reg_type == 'LR':
                final_graph_pruned = np.array(graph_prunned_by_coef(final_graph, training_set.inputdata))
            elif reg_type == 'QR':
                final_graph_pruned = np.array(graph_prunned_by_coef_2nd(final_graph, training_set.inputdata))
            elif reg_type == 'GPR':
                final_graph_pruned = np.transpose(pruning_cam(training_set.inputdata, np.array(final_graph).T))
            last_acc_est = count_accuracy(training_set.true_graph, final_graph.T)
            last_acc_est2 = count_accuracy(training_set.true_graph, final_graph_pruned.T)
            np.save('{}/final_graph.npy'.format(output_dir), np.asarray(final_graph))
            np.save('{}/final_graph_pruned.npy'.format(output_dir), np.asarray(final_graph_pruned))

        final_metrics = {
            "raw": last_acc_est,
            "pruned": last_acc_est2,
        }
        with open('{}/final_metrics.json'.format(output_dir), 'w', encoding='utf-8') as f:
            json.dump(final_metrics, f, indent=2, default=lambda o: float(o) if hasattr(o, "__float__") else o)
