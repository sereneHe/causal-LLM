...
CAMML_DATASETS=(${CAMML_DATASETS:-"${datasets[*]}"})
CAMML_DATASIZE1=(${CAMML_DATASIZE1:-"${datasize1[*]}"})
CAMML_DATASIZE2=(${CAMML_DATASIZE2:-"${datasize2[*]}"})
CAMML_CONFS=(${CAMML_CONFS:-"${conf_dict[*]}"})
CAMML_RUNS=(${CAMML_RUNS:-"1 2 3 4 5 6"})
CAMML_PRIOR_ROOT=${CAMML_PRIOR_ROOT:-"$REPO_ROOT/scripts/prior"}
CAMML_OUTPUT=${CAMML_OUTPUT:-"$CAMML_PRIOR_ROOT/CaMML_prior.txt"}
CAMML_DAG_PATH=${CAMML_DAG_PATH:-"$CAMML_PRIOR_ROOT/CaMML_record/prior"}

mkdir -p "$(dirname -- "$CAMML_OUTPUT")" "$CAMML_DAG_PATH"

cd "$REPO_ROOT/scripts/baseline/CaMML"

for j in "${!CAMML_CONFS[@]}"; do
	conf=${CAMML_CONFS[$j]}
	for i in "${!datasets[@]}"; do
		d=${datasets[$i]}
		anc=${ancs_dict[$i]}
		forb_anc=${forb_ancs_dict[$i]}
		for s in ${datasize1[$i]} ${datasize2[$i]}; do
			for r in ${CAMML_RUNS[@]}; do
				python CaMML_perform.py --d="$d" --N="$s" --ancs="$anc" --r="$r" --forb_ancs="$forb_anc" --output="$CAMML_OUTPUT" --dag_path="$CAMML_DAG_PATH" --conf="$conf" &
			done
		done
	done
	wait
